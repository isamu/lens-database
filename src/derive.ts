...
const isFisheye = (name: string) =>
  /fish[-\s]?eye/i.test(name) || /フィッシュアイ/.test(name);
...
const isMacro = (name: string) =>
  /(?:^|[\s\-])(?:macro|makro|mikro)(?:[\s\-]|$)/i.test(name) ||
...
const isSoftFocus = (name: string) =>
  /soft[-\s]?focus/i.test(name) || /\bthambar\b/i.test(name);
...
const SENSOR_DIAGONAL_MM: Record<Format, number> = {
...
const computeAngleOfView = (focalMm: number, format: Format) => {
  const diag = SENSOR_DIAGONAL_MM[format];
  if (!diag || !focalMm) return undefined;
...
const deriveAngleOfView = (
  data: LensData,
): number[] | undefined => {
  if (data.angleOfView) return data.angleOfView;
  // Skip fisheye — the geometric formula is wrong for it.
  if (data.hasFisheye || isFisheye(data.name)) return undefined;
  if (!data.focalLength?.length) return undefined;
  const values = data.focalLength.map((f) => computeAngleOfView(f, data.format));
  if (values.some((v) => v === undefined)) return undefined;
  return values as number[];
};

export const deriveLensData = (data: LensData): LensData => {
  const patch: Partial<LensData> = {};
  if (data.hasMacro === undefined && isMacro(data.name)) patch.hasMacro = true;
  ...
  const aov = deriveAngleOfView({ ...data, ...patch });
  if (aov !== undefined) patch.angleOfView = aov;
